...
interface ImageCardProps {
  image: ImageData;
  onSelect: (image: ImageData) => void;  // card click → sidebar detail
  onZoom: (image: ImageData) => void;    // zoom icon → full preview modal
  onDelete: (image: ImageData) => Promise<void>;
}

export default function ImageCard({ image, onSelect, onZoom, onDelete }: ImageCardProps) {
  ...
  return (
    <button
      ...
      className="group relative aspect-square w-full overflow-hidden rounded-xl bg-muted shadow-sm ring-1 ring-border/50 transition-all duration-200 hover:ring-2 hover:ring-primary/50 hover:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
    ...
    >
      ...
      {/* Hover overlay */}
      <div className="absolute inset-0 flex flex-col justify-between bg-gradient-to-t from-black/60 via-transparent to-transparent opacity-0 transition-opacity duration-200 group-hover:opacity-100">
